# Requirements Archive: v1.2 Home pressure trends card
...
## Milestone v1.2 — Shipped (2026-04-08)

### Home — combined trends card (HOME)

- [x] **HOME-01:** On Home, when the app shows trend data, the user sees **one** primary card that combines the former “latest reading” summary area and the “blood pressure over time” chart (no separate stacked card pair for that content).
- [x] **HOME-02:** The card headline shows **average** systolic and diastolic (mmHg) for the **same set of readings** represented in the chart (or the same time window), with an “Avg” (or equivalent) label per design reference; units shown clearly.
- [x] **HOME-03:** The chart in this card is a **bar chart** (not a dual line chart). Each bar corresponds to a **single saved reading** (or, if bucketing is required for density, requirements in the phase plan must state the aggregation rule; default is one bar per reading in the window). Paired systolic/diastolic values remain derivable from the underlying reading (e.g. tooltip, bar encoding, or legend — specified in plan).
- [x] **HOME-04:** Each bar’s fill color reflects that reading’s **risk band** using the product **Blood Pressure Chart** categories (hypotension blues, normal / acceptable **green**, pre-hypertension yellow, stage 1–4 / crisis orange–red). **Green** applies when the reading falls in the designated acceptable/normal bands (systolic and diastolic within the green-eligible ranges from the chart). When systolic and diastolic imply different bands, use the **more severe** band for color.
- [x] **HOME-05:** **Sparse / empty states** remain understandable: if there are fewer than two readings (or existing app threshold), the Home surface shows appropriate empty or hint UI **inside** the unified card layout (or a single empty card), not a broken split between two cards.
- [x] **HOME-06:** **Non-diagnostic** copy remains present on Home where the product already surfaces it (wording may reflow with layout merge; must not remove the medical-boundary message).
